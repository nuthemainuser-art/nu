...
export type NuUser = {
  ...
  username: string;
  name: string;
  passwordHash: string; // plaintext for now (demo only)
  keyId?: string;
  pin?: string;
};

// Minimal demo users – replace with DB later
let users: NuUser[] = [
  {
    id: "1",
    ...
    username: "demo",
    name: "Demo User",
    passwordHash: "demo1234", // use this for login in password mode
    keyId: "demo-key",
    pin: "1234",
  },
];
...
export function verifyPassword(email: string, password: string): NuUser | null {
  ...
  if (!user) return null;
  if (user.passwordHash !== password) return null; // later: bcrypt
  return user;
}
